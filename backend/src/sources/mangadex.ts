...
const HEADERS = { 'User-Agent': 'MangaApp/0.1 (portfolio reader)' };
...
async function getJSON<T>(url: string): Promise<T> {
  ...
  if (!res.ok) throw new Error(`MangaDex ${res.status} on ${url}`);
...
export class MangaDexProvider implements SourceProvider {
  ...
  async trending(options?: SearchOptions): Promise<MangaSearchResult[]> {
    ...
    if (options?.sort === 'latest') {
      p.append('order[latestUploadedChapter]', 'desc');
    } else {
      p.append('order[followedCount]', 'desc');
    }
